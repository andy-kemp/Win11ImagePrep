...
function Invoke-Cleanup {
    ...
    foreach ($img in $mountedImages) {
        try {
            Write-Host "Dismounting: $($img.Path)" -ForegroundColor Yellow
            Dismount-WindowsImage -Path $img.Path -Discard -ErrorAction SilentlyContinue
        } catch {
            Write-Host "Failed to dismount: $($img.Path)" -ForegroundColor Red
        }
    }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="$Title" Height="200" Width="500" WindowStartupLocation="CenterScreen" 
    ...
    <Grid Margin="20">
        ...
        <TextBlock x:Name="txtMessage" Text="$Message" Grid.Row="0" FontSize="14" Margin="0,0,0,15" TextWrapping="Wrap"/>
        <ProgressBar x:Name="progressBar" Grid.Row="1" Height="25" Minimum="0" Maximum="100" Value="0" Margin="0,0,0,10"/>
        <TextBlock x:Name="txtPercent" Grid.Row="2" Text="0%" HorizontalAlignment="Center" FontSize="12" Margin="0,0,0,10"/>
        <Button x:Name="btnCancel" Content="Cancel Operation" Grid.Row="3" Width="140" Height="28"/>
    </Grid>
</Window>
"@
...
# --- Multi-Driver Selection Dialog ---
function Show-MultiDriverDialog {
    param([array]$ExistingDrivers = @())
    
    $xaml = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="Select Driver Sources" Height="450" Width="700" WindowStartupLocation="CenterScreen" 
        ResizeMode="NoResize" Topmost="True" WindowStyle="SingleBorderWindow">
    <Grid Margin="15">
        <Grid.RowDefinitions>
            <RowDefinition Height="Auto"/>
        ...
        <TextBlock Grid.Row="0" Text="Add driver sources (MSI files or folders):" 
                   FontSize="14" Margin="0,0,0,10"/>
        <ListBox x:Name="lstDrivers" Grid.Row="1" Margin="0,0,0,10"/>
        <StackPanel Grid.Row="2" Orientation="Horizontal" Margin="0,0,0,10">
            <Button x:Name="btnAddMSI" Content="Add MSI File" Width="120" Margin="0,0,10,0"/>
            <Button x:Name="btnAddFolder" Content="Add Folder" Width="120" Margin="0,0,10,0"/>
            <Button x:Name="btnRemove" Content="Remove Selected" Width="120"/>
        </StackPanel>
        <StackPanel Grid.Row="3" Orientation="Horizontal" HorizontalAlignment="Center">
            <Button x:Name="btnOK" Content="OK" Width="120" Margin="0,0,10,0"/>
...
"@
    ...
    $reader = [System.Xml.XmlReader]::Create([System.IO.StringReader]$xaml)
    $window = [Windows.Markup.XamlReader]::Load($reader)
    $lstDrivers = $window.FindName("lstDrivers")
    $btnAddMSI = $window.FindName("btnAddMSI")
    $btnAddFolder = $window.FindName("btnAddFolder")
    $btnRemove = $window.FindName("btnRemove")
    $btnOK = $window.FindName("btnOK")
    $btnCancel = $window.FindName("btnCancel")
    
    $script:driversList = New-Object System.Collections.ArrayList
    
    # Pre-populate with existing selections
    foreach ($driver in $ExistingDrivers) {
        $item = New-Object System.Windows.Controls.ListBoxItem
        $item.Content = "[$($driver.Type)] $($driver.Path)"
        $item.Tag = $driver
        $lstDrivers.Items.Add($item) | Out-Null
        $script:driversList.Add($driver) | Out-Null
    }
    ...
    $btnAddMSI.Add_Click({
        ...
        if ($ofd.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            ...
            $isDuplicate = $false
            foreach ($existing in $script:driversList) {
                if ($existing.Path -eq $file) {
                    $isDuplicate = $true
                    [System.Windows.MessageBox]::Show("This driver is already in the list.", "Duplicate", "OK", "Warning")
                    break
                }
            }
            ...
            if (-not $isDuplicate) {
                ...
                $item = New-Object System.Windows.Controls.ListBoxItem
                $item.Content = "[MSI] $file"
                $item.Tag = $driverObj
                $lstDrivers.Items.Add($item) | Out-Null
                $script:driversList.Add($driverObj) | Out-Null
            }
        }
    })
    ...
    $btnAddFolder.Add_Click({
        ...
        if ($fbd.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            ...
            $isDuplicate = $false
            foreach ($existing in $script:driversList) {
                if ($existing.Path -eq $fbd.SelectedPath) {
                    $isDuplicate = $true
                    [System.Windows.MessageBox]::Show("This driver folder is already in the list.", "Duplicate", "OK", "Warning")
                    break
                }
            }
            ...
            if (-not $isDuplicate) {
                ...
                $item = New-Object System.Windows.Controls.ListBoxItem
                $item.Content = "[Folder] $($fbd.SelectedPath)"
                $item.Tag = $driverObj
                $lstDrivers.Items.Add($item) | Out-Null
                $script:driversList.Add($driverObj) | Out-Null
            }
    ...
    $btnRemove.Add_Click({
        if ($lstDrivers.SelectedIndex -ge 0) {
            $idx = $lstDrivers.SelectedIndex
            $script:driversList.RemoveAt($idx)
            $lstDrivers.Items.RemoveAt($idx)
        }
    })
    ...
    $btnOK.Add_Click({
        ...
        $uniqueDrivers = @()
        $seenPaths = @{}
        foreach ($driver in $script:driversList) {
            $path = $driver.Path
            if (-not $seenPaths.ContainsKey($path)) {
                $uniqueDrivers += $driver
                $seenPaths[$path] = $true
            }
        }
        $window.Tag = $uniqueDrivers
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="Windows 11 Image &amp; USB Creator V2" Height="720" Width="900" ResizeMode="NoResize" WindowStartupLocation="CenterScreen">
    <Grid Margin="10">
        <Grid.RowDefinitions>
            <RowDefinition Height="Auto"/>
            <RowDefinition Height="Auto"/>
        ...
        <TextBlock Grid.Row="0" Text="Windows 11 Image Preparation Tool - Version 2.0" 
        ...
        <StackPanel Grid.Row="1" Orientation="Horizontal" Margin="0,0,0,8">
            ...
            <Button Content="Verify ISO" x:Name="btnVerifyISO" Margin="5,0,0,0" Width="90" Height="24"/>
        </StackPanel>
        ...
        <StackPanel Grid.Row="3" Orientation="Horizontal" Margin="0,0,0,12">
            <Button Content="Advanced: Select Editions" x:Name="btnSelectEditions" Width="180" Height="28" Margin="0,0,10,0"/>
            <Button Content="Prepare Image with Drivers" x:Name="btnInject" Width="200" Height="28" Margin="0,0,10,0"/>
            <Button Content="Export to ISO" x:Name="btnExportISO" Width="140" Height="28" IsEnabled="False" Margin="0,0,10,0"/>
            <Button Content="Repair/Cleanup" x:Name="btnRepair" Width="140" Height="28"/>
        </StackPanel>
        
        <StackPanel Grid.Row="4" Orientation="Horizontal" Margin="0,0,0,8">
            <Button Content="Create from Saved Image" x:Name="btnFromSaved" Width="200" Height="28" Margin="0,0,10,0"/>
            <Button Content="Create USB from ISO" x:Name="btnUsbFromIso" Width="200" Height="28"/>
        </StackPanel>
        ...
        <StackPanel Grid.Row="6" Orientation="Horizontal" Margin="0,0,0,8">
            <TextBlock x:Name="lblStatus" Text="Ready" VerticalAlignment="Center" FontWeight="Bold" Foreground="Green" Margin="0,0,10,0"/>
...
$btnExportISO = $window.FindName("btnExportISO")
...
function Write-Status([string]$text, [string]$color = "Lime") {
    $timestamp = Get-Date -Format "HH:mm:ss"
...
function Set-StatusLabel([string]$text, [string]$color = "Green") {
    $lblStatus.Dispatcher.Invoke([action]{
        $lblStatus.Text = $text
...
$btnVerifyISO.Add_Click({
    ...
    Set-StatusLabel "Verifying ISO..." "Orange"
    $window.Cursor = 'Wait'
...
$btnExportISO.Add_Click({
    [System.Windows.MessageBox]::Show("ISO Export feature coming soon! This will create a new ISO file from the prepared Windows 11 image.", "Coming Soon", "OK", "Info")
})
...
$btnInject.Add_Click({
    ...
    $window.Cursor = 'Wait'
    Set-StatusLabel "Processing..." "Orange"
    $script:operationCancelled = $false
    ...
    try {
        ...
        if (-not $driverValidation.Valid) {
            throw "No valid drivers found in the selected sources"
        }
        ...
        foreach ($editionIndex in $editionsToProcess) {
            ...
            dism /Mount-Wim /WimFile:"$installWim" /index:$editionIndex /MountDir:"$mountEdition" | Out-Null
            dism /Image:"$mountEdition" /Add-Driver /Driver:"$driversDir" /Recurse | Out-Null
            ...
            if (Test-Path $winreWim) {
                ...
                dism /Mount-Wim /WimFile:"$winreWim" /index:1 /MountDir:"$mountWinRE" | Out-Null
                dism /Image:"$mountWinRE" /Add-Driver /Driver:"$driversDir" /Recurse | Out-Null
        ...
        Set-StatusLabel "Image Ready" "Green"
        $btnExportISO.IsEnabled = $true
...
# Startup message
Write-Status "Windows 11 Image Preparation Tool V2 - Ready"
Write-Status "Features: Multi-driver support, Edition selection, Progress tracking, Error recovery"
Set-StatusLabel "Ready" "Green"
